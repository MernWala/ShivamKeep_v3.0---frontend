...
import { useLoginMutation, useLoginWithTokenQuery, useRegisterMutation } from '../store/rtk/auth.api';
...
export function Auth() {
    ...
    try {
      if (authType === 'Login') {
        ...
        if (result?.status === 201) {
          alert('Check email for id validation');
      ...
      if (isFetchBaseQueryError(error)) {
        if (error.status === 498) {
          ...
          alert((error.data as { message: string }).message);
        } else {
          alert('Something went wrong, try after sometime!');
        }
      } else if (error instanceof Error) {
        alert(error.message || 'Something went wrong, try after sometime!');
      } else {
        alert('Something went wrong, try after sometime!');
      }
    } finally {
  ...
  const handleForgotPassword = () => {
    navigate(
      `/account/verify-email?access=AccountRecovery${formData.email ? `&email=${encodeURIComponent(formData.email)}` : ''}`
    );
  };
        ...
        <div className="w-full rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            <h1 className="text-center text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
              {authType === 'Login' ? 'Sign in to your account' : 'Create your free account'}
            </h1>
              ...
              <div>
                ...
                {authType === 'Login' && (
                  <div className="flex items-center justify-between mt-2">
                    <button type="button" className="border border-transparent focus:border-white rounded-[3px] text-sm font-medium text-primary-600 hover:underline dark:text-primary-500 focus:outline-none" onClick={handleForgotPassword}>
                      Forgot password?
                    </button>
                  </div>
                )}
